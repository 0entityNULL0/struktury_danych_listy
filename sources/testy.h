...
template <typename T, typename B> int testuj_add_on_position(T &struktura, B &dodawany_przedmiot, unsigned int ile)
{
    ...
    std::srand(std::time({}));
    unsigned int* losowe_pozycje = new unsigned int[ile];
...
void wykonaj_szukanie(tablica_dynamiczna<T> &t, B szukany) {
    unsigned int s = t.size();
    for(unsigned int j = 0; j < s; j++) {
        if(t[j] == szukany) break;
    }
}
...
template <typename T, typename B> int testuj_find(T &struktura, B szukany, unsigned int ile)
{
    ...
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    for(int i = 0; i < ile; i++) {
        wykonaj_szukanie(kopie[i], szukany);
    }
    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    delete[] kopie;
    return std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count();
...
void typowy_test(unsigned int ile, unsigned int rozmiar, std::string nazwa_struktury = "Struktura")
{
    ...
    long long find_time = testuj_find(tablica, szukany, ile);
